...
const UserTable = forwardRef((props, ref) => {
  ...
  const { listUsers, isLoading, refetchUsers, dataUser, setUserId } =
    useUsers();
  ...
  const columns = [
    ...
    },
    {
      ...
      onFilter: (value, record) => {
        if (value === ">") return +record.price > 1500;
    ...
    <>
      <div style={{ display: "flex", justifyContent: "space-between" }}>
        <UserDeleteMany
          ...
          setUserIds={setArrUserIds}
          refetchUsers={refetchUsers}
        />
        <ButtonComponent
          onClick={() =>
            exportFileExcel(listUsers, "Products", "users -table.xlsx")
          }
          text={"Export .xlsx"}
          icon={<ExportOutlined />}
